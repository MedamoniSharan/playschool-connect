...
const colorMap: Record<string, string> = {
  ...
  gallery: "bg-primary-light text-dash-lime-deep",
  attendance: "bg-success/10 text-success",
  announcement: "bg-primary-light text-dash-lime-deep",
      ...
      <div className="space-y-3">
        {filtered.map((n) => (
          <div key={n.id} onClick={() => markRead(n.id)}
            className={`bg-dash-surface rounded-[24px] border border-dash-subtle p-4 flex items-start gap-4 cursor-pointer transition-all hover:shadow-sm ${
              !n.read ? "border-l-4 border-l-primary" : ""
            }`}>
            <div className={`w-9 h-9 rounded-[16px] flex items-center justify-center flex-shrink-0 ${colorMap[n.type]}`}>
              {iconMap[n.type]}
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start gap-2">
                <p className={`text-sm ${!n.read ? "font-semibold" : "font-medium"}`}>{n.title}</p>
                <span className="text-xs text-dash-muted whitespace-nowrap">{n.date}</span>
              </div>
              <p className="text-sm text-dash-muted mt-0.5">{n.message}</p>
            </div>
          </div>
        ))}
        {filtered.length === 0 && (
          <div className="text-center py-16 text-dash-muted"><p>No notifications</p></div>
        )}
      </div>
    </div>
  );
